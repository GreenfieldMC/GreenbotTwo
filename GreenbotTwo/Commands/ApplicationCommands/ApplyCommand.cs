...
public class ApplyCommand(IApplicationService applicationService, IGreenfieldApiService apiService, IAccountLinkService accountLinkService, ILogger<IApplicationCommandContext> commandLogger) : ApplicationCommandModule<ApplicationCommandContext> 
{
    ...
    public async Task Apply()
    {
        ...
        if (!isAppInProgress)
        {
            commandLogger.LogInteractionDebug(Context, $"No in-progress application found for user {Context.User.Username} ({Context.User.Id}). Attempting to find linked accounts.");
            ...
            {
                commandLogger.LogInteractionDebug(Context, $"Failed to fetch linked accounts for user {Context.User.Username} ({Context.User.Id}). Status code: {connectionWithUsersResponse.StatusCode}, Error: {connectionWithUsersResponse.ErrorMessage}");
            ...
            if (users.Count == 0)
            {
                commandLogger.LogInteractionDebug(Context, $"No linked accounts found for user {Context.User.Username} ({Context.User.Id}). Checking cache for verified users.");
                var cached = accountLinkService.GetCachedVerifiedUser(Context.User.Id);
                if (cached is not null)
                    users.Add(cached);
            }
            
            commandLogger.LogInteractionDebug(Context, $"Found {users.Count} linked accounts for user {Context.User.Username} ({Context.User.Id}).");
        ...
        commandLogger.LogInteractionDebug(Context, $"Resuming in-progress application for user {Context.User.Username} ({Context.User.Id}).");
    ...
    public async Task Cancel()
    {
        ...
        {
            commandLogger.LogInteractionDebug(Context, $"User {Context.User.Username} ({Context.User.Id}) attempted to cancel an application, but none was found.");
        ...
        {
            commandLogger.LogInteractionDebug(Context, $"User {Context.User.Username} ({Context.User.Id}) attempted to cancel a submitted application.");
        ...
        commandLogger.LogInteractionDebug(Context, $"User {Context.User.Username} ({Context.User.Id}) cancelled their in-progress application.");
